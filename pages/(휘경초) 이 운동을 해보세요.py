...
age = st.number_input("나이 (세)", min_value=6, max_value=12, step=1)
gender = st.selectbox("성별", ["남자", "여자"])
activity_level = st.selectbox("활동 수준", ["낮음", "보통", "높음"])
health_condition = st.text_input("건강 상태 (예: 천식, 알러지 등)")
exercise_preference = st.text_input("운동 선호도 (예: 축구, 수영 등)")
