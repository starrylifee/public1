...
if uploaded_file is not None:
  ...
  # Generate content
  response = model.generate_content(["이 사진은 책상위에 올려둔 나침반의 바늘이 움직이다 멈추었을 때 어느방향을 가르키는지 쓰고, 그 까닭을 그림으로 나타낸 작품입니다. 평가기준 1: '자석의 N극은 북쪽, S극은 남쪽을 가리킨다'는 그림이 들어가 있으면 10점을 주고, 둘중 하나만 있는 경우에는 5점을 주세요. 평가기준 2: '지구가 하나의 자석과 같은 성질이 있다, 지구의 북극쪽이 S극 지구의 남극쪽이 N극의 성질을 나타낸다'는 그림이 들어가면 추가로 10점을 주세요. 평가기준은 2개이며 각각 10점 만점으로 총점 20점 만점입니다. 평가기준과 기준별 획득점수도 출력해주세요.", img])
